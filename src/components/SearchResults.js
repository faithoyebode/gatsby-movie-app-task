    ...
    const handleSubmit = async (e) => {
        try {
            ...
            const [ moviesResults, seriesResults ] = await Promise.all([
                    fetch(`https://omdbapi.com/?apikey=${process.env.GATSBY_API_KEY}&s=${e.target.value}&type=movie&page=1`),
                    fetch(`https://omdbapi.com/?apikey=${process.env.GATSBY_API_KEY}&s=${e.target.value}&type=series&page=1`)
            ]);
        ...
        if((e.target.scrollLeft + e.target.clientWidth >= e.target.scrollWidth - 30) && (stateItem.page * 10 < Number(stateItem.totalResults))){
            if((!moviesScrolled && category === 'movie') || (!seriesScrolled && category === 'series')){
                try{
                    ...
                    const response = await fetch(`https://omdbapi.com/?apikey=${process.env.GATSBY_API_KEY}&s=${searchTextRef.current.value}&type=${category}&page=${page}`);
